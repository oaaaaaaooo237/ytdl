...
后续用户确认修正：第一阶段不能取消或推迟 ffmpeg 合并、字幕嵌入和字幕烧录。本文第一轮第 3 项中的 ffmpeg 延后建议不再作为项目口径；最新权威口径以设计文档为准：ffmpeg Android 集成是第一阶段必须完成并真实验证的能力，法律、体积、ABI 和 Play 风险审计属于第一阶段准入任务。
...
3. ffmpeg 设计前后不一致：技术路线说打包 Android ABI ffmpeg，MVP 又说只做检测和能力说明。后续用户已明确第一阶段必须包含 ffmpeg 合并、字幕嵌入和字幕烧录；因此应把 ffmpeg 集成、许可、体积、ABI 和 Play 风险审计作为第一阶段优先任务。
...
- 明确第一阶段必须内置或随包集成可审计的 Android ffmpeg 能力，并把合并、字幕嵌入和字幕烧录纳入第一阶段验收。
...
- ffmpeg 第一阶段边界已按用户后续确认修正：必须承诺合并、字幕嵌入和字幕烧录真实执行。
...
- 实现计划需要把 Android 工程结构、Chaquopy 验证、Android ffmpeg 方案验证、前台服务真实下载、MediaStore/SAF 导出作为前五个技术风险优先任务。
