...
- 明确媒体处理阶段边界：MVP1 使用原生 `MediaMuxer` 合并和独立字幕文件；MVP2 再纳入字幕嵌入、字幕烧录和三合一输出。
...
- 媒体处理边界已按 2026-06-20 用户确认再次修正：MVP1 承诺原生合并与独立字幕文件，字幕嵌入和字幕烧录移入 MVP2。
...
- 实现计划需要把 Android 工程结构、Chaquopy 验证、原生媒体合并、独立字幕文件、前台服务真实下载、MediaStore/SAF 导出作为优先技术风险任务。
