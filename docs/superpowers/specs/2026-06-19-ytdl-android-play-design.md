...
- GUI 布局基准图：`docs/android-gui-reference-v3.png`。该图为用户确认的第三版 Android 方向，后续实现应尽可能还原其布局、页面组织和交互骨架。
...
- 色彩默认采用 `docs/android-gui-reference-v3.png` 中的配色；如后续换色，只允许两类策略：一是参考 Codex 主题配色统一设计 5 个页面，二是分别设计 5 个页面各自的背景色、前景色和强调色。
- 首页优先完成真实操作，不做宣传首屏。
