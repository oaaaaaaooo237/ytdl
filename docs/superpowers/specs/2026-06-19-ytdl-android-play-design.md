...
- 第一阶段 Play MVP 只承诺不需要 ffmpeg 的下载路径：单文件音视频、仅音频原始保存、仅视频原始保存、字幕文件下载。
- 需要 ffmpeg 的合并、转码、嵌入和烧录在第一阶段只显示能力缺失或后续支持说明，不作为可执行承诺。
- 内置 ffmpeg 必须经过独立法律、体积、ABI 和 Play 审核风险审计后，才能进入第二阶段实现计划。
...
- ffmpeg：第一阶段不内置可执行合并能力，只设计 `MediaProcessor` 接口和缺失态；第二阶段再决定是否按 Android ABI 打包经过许可审查的 ffmpeg 可执行文件或库。不要依赖已退役的 FFmpegKit 作为长期基础。
