...
- 第一阶段 Play MVP 必须包含 ffmpeg 相关真实能力：高分辨率分离音视频合并、字幕嵌入和字幕烧录。
- 单文件音视频、仅音频原始保存、仅视频原始保存、字幕文件下载仍作为不依赖 ffmpeg 的基础路径保留。
- ffmpeg 集成不得运行时下载或替换二进制；必须随应用版本打包、记录许可证、统计包体积，并在 `arm64-v8a` 真机和 `x86_64` 模拟器上真实验证。
- 如果 ffmpeg 集成尚未通过法律、体积、ABI 和 Play 审核风险审计，则 Android 第一阶段不能视为完成；审计是第一阶段内的准入任务，不是推迟功能的理由。
...
- 设置：默认保存位置、cookies 文件、解析器版本、ffmpeg 能力状态、隐私和法律说明。并发数设置不进入第一阶段。
...
- ffmpeg：第一阶段必须实现 Android 端 `MediaProcessor`，并随应用打包经过许可审查的 Android ABI ffmpeg 可执行文件或库，用于音视频合并、字幕嵌入和字幕烧录。不要依赖已退役的 FFmpegKit 作为长期基础；具体采用自建 ffmpeg、可信维护 fork 或等价可审计方案，应在实现计划中作为优先技术风险任务确定。
...
- app 私有目录保存成功。
- SAF 导出成功。
- ffmpeg 在 Android `arm64-v8a` 和 `x86_64` 环境可被调用。
- 真实执行高分辨率分离音视频下载后合并。
- 真实执行字幕文件下载、字幕嵌入和字幕烧录。
...
- 高分辨率音视频分离格式可通过 Android ffmpeg 合并为可播放文件。
- 字幕文件下载、字幕嵌入和字幕烧录真实可用。
- 前台服务通知真实进度。
- 历史记录写入 Room。
- 通过系统文件选择器导出文件。
