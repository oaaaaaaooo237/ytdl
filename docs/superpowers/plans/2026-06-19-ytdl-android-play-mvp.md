...
这份清单是当前 Android 交付主线；按顺序逐项推进，每项完成后先验证，再提交到当前分支，再进入下一项。Computer Use 前台操作与观察是 Android GUI 验收依据；不为留档重复真实下载。

- [x] D0：移除截图/XML作为 M11 功能验收门槛，删除本轮新增的辅助文件，并把相关计划和 QA 口径统一为 Computer Use 前台观察。
- [x] D1：完成 M11 当前五页前台复核：下载、格式、队列、历史、设置；确认 `基准图配色` 与 `Codex 风格` 可切换并影响页面配色；M11 总验收仍保留后续差异收敛门槛。
...
- [ ] D2：M10 真机门槛（进行中）：已在 Xiaomi 14 `arm64-v8a` 真机完成安装、五页 GUI、通知授权、真实分析/下载/原生合并和后台完成观察；当前代码续作完成后，只需复核地址栏 1 至 5 行、全幅图标、保存位置、自动外部保存后的中转清理、历史打开/分享和审慎缓存清理。手动导出已按用户要求取消，不再列入验收；不为存储行为重复真实下载。
...
- 未解决问题 4：分析结果仅提供单文件视频、没有可分离的独立音频流和视频流时，`视频+音频` 应灰显且不可选；`视频下载` 应只列真实可下载格式，选中后可以下载。当前模式可用性和格式可下载性均不正确。专项测试地址为 `https://www.eporner.com/video-cDSGZsgq7rb/transfixed-muscle-hunk-gets-buttfucked-by-two-horny-trans-girls-kasey-kei-and-bella-joie/?trx=1227735290aee694b81473a256bea12420712` 与 `https://www.eporner.com/video-Wyzh97cKNIY/bella-gets-told-do-and-spreads-em-bella-rolland-milan-ponjevic/`；控制真实请求频率，不测试字幕。
- 待优化 1：地址栏已有输入时，长按应弹出至少包含全选、复制、粘贴和删除的快捷菜单，并能正确操作当前内容。
- 未解决问题 6：手机版缺少电脑版已有的启动后 `yt-dlp` 最新版本比对，设置页“解析器版本”点击无反应。修复后启动检查必须异步且不阻塞；发现新版时提示下载或更新应用；点击版本行可以主动重查；不得运行时替换内置 Python 代码。
- 设置交互缺口：设置页“默认保存位置”“解析器版本”“媒体处理能力”“地址校验提示”必须分别接入真实保存位置选择、更新检查、媒体能力说明和地址校验说明。
- 剩余检查：修复并复核问题 1-4、问题 6、优化 1 和四个设置入口；确认本次完成记录进入历史；打开合并视频；导出和分享；最后删除本次测试历史并精确清理 App 私有测试输出目录。当前未清理，关机后恢复时先保留该任务供后续检查。
- 状态：D2/M10 尚未完成；本次按用户要求暂停，不继续操作真机。

#### 2026-07-13 剩余验收顺序调整

用户将剩余工作明确拆为两个独立前台关卡，不能再把模拟器与真机验收合并表述：

- [x] 第 5 步：当前问题修复和自动化验证完成后，安装最新 debug APK 到单一可见 API37 模拟器，使用 Computer Use 操作前台 GUI。开始前先执行规定的 `node_repl` 最小 smoke，再执行 `sky.list_apps()`；计算器不是必需前置。验收范围包括地址栏真实长按文字菜单、保存位置入口、解析器版本主动检查、媒体处理能力说明、地址校验说明、缓存统计与确认清理、应用图标、通知点击返回、单文件媒体模式/格式可用性和测试数据清理。不得用 adb、UIAutomator、connected test 代替前台操作；它们只能辅助确认状态。缓存清理优先使用合成 App 私有测试文件，不为留档重复 YouTube 下载；问题 4 只使用两条 Eporner 专项地址并控制真实请求频率，不测试字幕。第 5 步完成后暂停。
- [ ] 第 6 步：在小米 14 `arm64-v8a` 真机上执行最终 Computer Use 验收。沿用已认可的历史真实下载结果，不为证明重复主地址下载；复核第 5 步修复、地址栏 1 至 5 行、全幅图标、现有完成记录、打开、分享、通知返回、保存位置和审慎缓存清理。手动导出已取消；存储生命周期由代码和自动化边界测试确认，不以重复真实下载证明。最后只清理本轮明确识别的测试历史和临时过程文件，不删除用户完成文件。只有该步完成后才能宣称 M10/D2 真机验收通过。

第 5 步完成记录（2026-07-14）：Computer Use 在唯一可见 API37 模拟器中完成正式图标、地址栏原生长按菜单、系统保存位置选择器、解析器版本、媒体能力、地址校验、缓存统计/确认清理、通知点击返回和单文件媒体专项检查。首轮发现“解析器版本缺少状态对话框”和“明确选择 `240p` 仍下载自动 `720p`”两项缺陷；以 TDD 修复并完成 3 项 P2 审计闭环，第二轮独立复审 `CLEAN`。受影响测试 `112/112`、相关组 `179/179`、全量单测 `295/295` 和 debug 构建通过。修复后前台复验显示解析器状态对话框及重新检查可用，第二条专项地址的具体 `240p` 选择进入真实队列并显示 `83.8 MB`，随后立即取消。两轮合计专项分析 3 次、下载启动 2 次、下载完成 0 次、字幕请求 0 次；本轮历史、通知、地址分析态和 App 私有文件均已清理。按用户要求完成后暂停，不提前执行第 6 步。

#### 2026-07-14 保存生命周期与优化 1/2/3 续作

执行目标按以下顺序收口：

1. 保存生命周期：SAF 外部产出成功后，历史记录精确的 `content://` 地址并立即删除 App 私有任务目录；App 私有默认目标保留最终成品，合并成功后立即删除中间视频/音频流。
2. 审慎缓存清理：只删除 App 私有下载根目录内未被队列或历史引用的失联过程文件；活动下载时禁止清理；不得删除用户完成文件、外部文件、数据库或程序资源。
3. 优化 1：地址栏使用原生 1 至 5 行自适应换行，保留软键盘与原生长按文字菜单。
4. 优化 2：用户选择的 C 版 3D 图标保持配色和主体，改为四边铺满、无白色异形背景的正方形图标。
5. 优化 3：取消历史页手动导出及相关启动器和声明；打开/分享必须同时支持 App 私有文件和外部 `content://` 文件。
6. 完成后执行一轮 fresh 独立只读代码审计，重点检查重复生命周期逻辑、旧导出声明、死代码、删除边界和过度实现；修正有效发现后，顺序运行聚焦回归、全量 `:app:testDebugUnitTest`、`:app:assembleDebug` 和可见 Android Computer Use 前台验收。

存储生命周期通过代码与自动化边界测试确认，不重复触发真实下载；可见前台只复核新增界面与交互，截图/XML不是阻塞项。

模拟器与真机的地址输入统一采用前台软键盘逐键点击：开始前先观察并记下字母键位、符号键盘入口和目标符号位置；相邻按键间隔不得少于 `0.18 秒`，每次键盘页面切换后至少等待 `0.5 秒`。完整地址一次性输入完后再统一校验，校验准确前不得点击“分析”。

#### 保存生命周期与优化 1/2/3 完成记录（2026-07-14）

- 实现已收口：外部 SAF 产出成功后历史记录精确 `content://` 地址并清理 App 私有任务目录；默认 App 私有目标保留完成文件；合并成功后删除中间流；历史页移除手动导出；打开/分享兼容 App 私有文件和外部 URI；地址栏为原生 1 至 5 行；启动器采用铺满图标区域的 C 版自适应图标。
- 独立审计修复了完成媒体保护、清理失败可见性、旧导出残留、符号链接越界和部分清理误报成功等有效发现。符号链接回归、外部哨兵保护和删除失败可重试均有自动化约束；所有已完成审计子任务均已关闭。
- 最终顺序验证通过：`:app:testDebugUnitTest --no-parallel` 共 `310/310`，0 失败、0 错误、0 跳过；`:app:assembleDebug --no-parallel` 成功。
- Computer Use 在唯一可见 API37 模拟器完成前台复核：C 版图标无白色异形托底；地址栏按每键至少 `0.18 秒`、页面切换至少等待 `0.5 秒` 的规则一次输入长地址并在结束后统一核对，准确换为 3 行；历史页无导出入口；保存位置主体打开系统目录选择器；缓存确认文案明确保护完成文件和所选目录文件。本轮未点击分析，未触发真实网络、下载或字幕测试。
- 该记录完成本节第 1 至 6 项，但不替代第 6 步小米 14 真机最终验收；D2/M10 仍保持进行中。

#### Eporner 格式编码、历史刷新与失败重试续作（2026-07-14）

1. 视频下载格式按分辨率分组，每个分辨率只显示一行；右侧列出实际编码选项，例如 `H.264`、`AV1`、`VP9`、`H.265`。默认优先 `H.264/AVC`。同分辨率、同编码存在多个底层格式时，内部优先含音频单文件，再比较帧率和质量。用户最终选中的编码必须映射到精确 format id；若该格式失效，提示重新分析，禁止自动切换编码、分辨率或自动格式。
2. 队列和历史卡片右下角显示上下两个同规格徽标：上方为实际下载编码，下方为分辨率；没有可靠值时不显示对应徽标，不得写死。
3. 任务进入完成、失败或取消终态时，正在显示的历史页立即刷新；重新进入历史页仍从 Room 刷新。
4. 失败任务在队列显示 `重试`，在历史显示 `再次下载`。点击后用原 URL 和当前 cookies 设置重新分析，成功后进入格式页；上次下载模式、分辨率和编码对应的精确 format id 仍可用时自动恢复，否则回落到当前分析结果中的兼容默认项。用户确认或调整后才创建新任务，禁止点击重试后直接下载；原失败记录保留。
5. 历史重试跨 App 重启保留。完整 URL 和上次路由不得进入 Room 历史、日志或 UI，只允许以最小草稿形式加密保存在 App 私有且不参与备份的重试存储；不保存标题、缩略图、格式摘要、cookies 或字幕选择。删除历史时同步删除，完成记录不创建；损坏或旧版本草稿不得显示 `再次下载`。
6. TDD 顺序：格式分组和编码选择；精确路由与编码徽标；历史终态刷新；队列重试；历史加密持久化与删除边界。每项先确认旧实现失败，再做最小修复。
7. 验证顺序：聚焦测试、相关 UI/隐私测试、全量 `:app:testDebugUnitTest --no-parallel`、`:app:assembleDebug --no-parallel`。最后需将任务切换到“完全访问权限”，用 Computer Use 在可见 API37 模拟器复核；真实下载控制频率，不测试字幕，不为留档重复下载。
8. 从本版开始，格式、队列、历史、设置说明等前台页面隐藏字幕相关选项、阶段、产物和动作；GUI 创建下载请求时固定不选择字幕。后端请求模型和下载管线暂时保留字幕能力，但本轮不提供前台入口，也不参与默认下载。

暂停记录（2026-07-14，按用户要求完成五步清单第二步后暂停）：

- 第二步已完成：视频下载按分辨率单行分组，右侧显示真实编码选项；默认优先 `H.264`，显式选择保留精确 format id，同编码多格式内部优先单文件、帧率和大小。格式页 Compose 点击 `AV1` 已验证精确选择 `av1-1080`。
- 已提前完成第三步的编码徽标子项：安全格式摘要记录明确编码；队列和历史共享右侧徽标组件，状态在右上，编码位于右下区域上方，分辨率在最下方；未知编码和旧内部 ID 不猜测。队列位置/尺寸测试已通过，历史专门映射与卡片测试仍待补。
- 历史即时刷新根因已定位但未改生产代码：`DisposableEffect(Unit)` 中的下载监听器捕获初始 `selectedRoute`。恢复时先用 `rememberUpdatedState(selectedRoute)` 做红绿测试并修复。
- 失败任务的队列 `重试`、历史 `再次下载`、跨重启加密重试请求存储和删除边界均未开始。
- 本暂停点聚焦回归共 22 项通过；尚未针对本次增量运行全量 `:app:testDebugUnitTest --no-parallel`、`:app:assembleDebug --no-parallel` 或 Computer Use。
- 恢复顺序：历史编码专门测试 -> 历史终态即时刷新 -> 队列重试 -> 历史加密持久化/删除 -> 聚焦与全量验证 -> 可见 API37 模拟器 Computer Use。真实下载继续控频，不测试字幕。

续作纠偏（2026-07-14）：此前“重试直接创建下载任务”的方向已废止。当前实现改为重新分析原 URL、进入格式页并尽量恢复上次选择；加密存储收敛为 URL 与路由，旧版本或损坏草稿不显示重试入口。前台字幕入口、阶段、产物描述和动作已隐藏，GUI 下载固定不选择字幕；后端字幕能力保留。已完成聚焦回归，仍需本轮全量单测、debug 构建和可见模拟器 Computer Use 前台复核后才能关闭该续作。
...
#### Android 1.0.1 任务与下载体验完成记录（2026-07-15）

- 1.0.1 已把“队列”和“历史”合并为“任务”，并把独立“格式”页合并到下载页，底部最终为“下载 / 任务 / 设置”三项。任务页运行时沿用实时下载卡片，结束后改用历史卡片；取消记录支持“再次下载”。
- 下载进度行现在同时显示实时速度、已下载大小和本阶段固定总大小。等待队列按加入顺序执行，当前任务取消、失败或完成后，下一条自动开始；同时仍只有一个下载任务执行。1.0.1 的等待队列保存在进程内，系统强制结束进程后未开始的任务不会自动恢复。
- 直接下载、仅视频、仅音频和原生合并后的主文件均按解析出的媒体标题命名；历史第二行显示主文件名和时间，不再显示 `web`、“媒体文件”、重复分辨率或完整路径。编码和分辨率保留在右侧徽标中。
- API37 可见模拟器已完成 YouTube 视频+音频、Eporner 单视频、Pornhub 仅分析、第二条任务等待后自动接续、取消后再次下载、解析器删除和重新下载等前台流程。问题 8 又以前台可见方式确认历史重试直接回到下载页，并依次切换视频+音频、视频、音频；音频模式的视频项目灰显，真实音频格式和容器可见。本次只分析，没有重复下载媒体。Python `267/267`、Android JVM `377/377` 和 debug/release/androidTest 构建均通过。
- 当前版本为 `versionCode=2`、`versionName=1.0.1`。小米 14 在发布收尾时未出现在 adb 设备列表中，最新 APK 尚未重新安装到真机；因此 M10/D2 仍为进行中，不能把 API37 模拟器结果写成真机最终验收通过。
