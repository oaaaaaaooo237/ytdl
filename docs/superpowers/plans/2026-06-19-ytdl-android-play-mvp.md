...
- 1.0.1 已把“队列”和“历史”合并为“任务”，底部改为“下载 / 格式 / 任务 / 设置”四项。任务页运行时沿用实时下载卡片，结束后改用历史卡片；取消记录支持“再次下载”。
...
- API37 可见模拟器已完成 YouTube 视频+音频、Eporner 单视频、Pornhub 仅分析、第二条任务等待后自动接续、取消后再次下载、解析器删除和重新下载等前台流程。Python、Android 全量单测和 debug/release/androidTest 构建均通过。
