...
- [ ] D2：M10 真机门槛（进行中）：已在 Xiaomi 14 `arm64-v8a` 真机完成安装、五页 GUI、通知授权、真实分析/下载/原生合并和后台完成观察；仍需处理本节暂停记录中的问题 1-4、问题 6、优化 1 和设置交互缺口，并继续历史完成记录、打开、导出、分享和清理检查。恢复时沿用现有完成任务；只有问题 4 使用用户指定的两个单文件媒体专项地址做受控真实测试。
...
- [ ] 第 6 步：明天在小米 14 `arm64-v8a` 真机上执行最终 Computer Use 验收。沿用已认可的历史真实下载结果，不为证明重复主地址下载；复核第 5 步修复、现有完成记录、打开、导出、分享、通知返回、保存位置和缓存清理，必要的单文件媒体真实请求仍只使用两条 Eporner 专项地址。最后删除本次测试历史并精确清理 App 私有测试输出。只有该步完成后才能宣称 M10/D2 真机验收通过。
