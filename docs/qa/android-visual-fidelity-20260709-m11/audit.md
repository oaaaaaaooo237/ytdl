...
Computer Use 后续已恢复到可见 YTDL 窗口并看到一张真实进行中队列卡，但该历史链接任务在原生合并时因模拟器空间不足失败；未留下符合本审计要求的可保存新地址截图。已修复容量预检和失败中间流清理，且已删除该任务及遗留文件。后续新地址真实测试仍需保存正式截图，不能以本次临时观察替代 M11 证据。

## 2026-07-10 格式可见性口径

分析后的格式页仅列当前视频实际提供的高度，完全不存在的标准高度不再显示为灰色“未提供”。检测到但不兼容原生 MP4 合并的高度仍保留并显示原因。该规则已经过 JVM/Compose 绑定回归和 debug 打包；尚待前台截图复核。

## 2026-07-10 审计修复边界

fresh audit 发现取消下载时已落盘、但尚未成为最终输出的视频流可能残留。取消分支现已复用受控未跟踪文件清理，JVM 回归确认任务目录为空；完成媒体仍受保留规则保护。该修复不形成新的前台视觉证据，后续真实取消路径需在恢复 Computer Use 后复核。

## 2026-07-10 前台格式与失败恢复观察

可见 API37 窗口显示：没有真实流的固定高度不再出现，实际存在但不兼容原生 MP4 合并的行仍显示原因；可合并行和单文件行均保持真实标记。一次真实无字幕任务显示下载视频、下载音频、取消动作、右下 `1080p` 和真实字节进度，随后以准确的存储空间不足消息失败；失败与分辨率徽标继续遵守右上/右下规则。该会话内观察尚未另存为审计截图文件，不能替代 M11 既定的落盘截图要求。

## 2026-07-10 容量误报更正

该次新主地址失败的后续辅助核验显示 `/data` 约有 `550 MB` 可用空间，而流输入约为 `87.4 MB + 13.9 MB`。前台“存储空间不足”提示由探测目标错误引起：合并器对尚不存在的输出文件查询 `usableSpace`，可得到 `0`。现在容量探测改为针对已创建的受控输出目录；新增合同回归先复现旧目标、再保护目录目标。全量 debug 单测和 debug 打包均通过，APK 已重新安装并能在可见 API37 模拟器前台启动。

这不是新的前台合并成功证据：完整真实下载仍处于 30 分钟节流窗口内，待窗口允许后才可用主/备用地址复验。M11 继续未通过。

## 2026-07-10 审计 P2 处置

独立审计指出目录目标 JVM 测试不能单独证明真实 MediaMuxer 成功。已将 API37 本地生成媒体 instrumentation 的成功输出改为原本不存在的 `new-task/merged.mp4`，并断言受控目录创建、输出非空和各一条视频/音频轨。`NativeMuxerMediaProcessorInstrumentedTest` 两项通过，全量 debug 单测与 debug 打包随后再次通过。该设备级、本地无网络覆盖关闭 P2，但不构成新的前台真实下载或 M11 完成证据。

## 2026-07-10 容量复验与历史删除可见性

可见 API37 模拟器中，新主地址完成了无字幕视频+音频下载：队列先显示真实 `19.3 MB / 87.4 MB` 和 `1080p`，随后通过原生合并，完成态显示三阶段勾选与 `101.5 MB / 101.5 MB`。此前的容量假拒绝未再出现。此会话的前台画面未另存为独立审计包。

完成历史卡最初未显示删除操作，无法满足每次真实测试后前台删除记录的纪律。删除 chip 改为独立可见行后，最新版 APK 前台显示删除、确认对话和空历史；辅助清理已确认的测试任务目录后 `files/gui-downloads` 为空。M11 仍缺完整五页的落盘截图级对照。

提交后 API37 connected 删除确认回归通过，覆盖测试记录取消删除后保留、再次确认后消失。该无网络辅助证据补足回调运行时验证，不替代本节已完成的前台可见删除。

## 2026-07-10 当前主地址真实队列与五页复核

Computer Use 在可见 API37 模拟器中逐键输入完整主地址 `https://www.youtube.com/watch?v=PqQNXB6hhUs`，辅助 UIAutomator 树核验完整字段。真实分析后，下载页、格式页、队列页、历史页和设置页均可前台切换；设置页滚动后 `外观与颜色`、`基准图配色`、`Codex 风格` 位于底栏上方。无字幕视频+音频真实任务在队列中出现 `39.2 MB / 187.9 MB`、`6%`、取消动作和 `1080p`，随后出现 `107.1 MB / 187.9 MB`、`19%`，最终完成态显示三阶段勾选、`196.3 MB / 196.3 MB`、绿色“完成”和右下 `1080p`。

历史记录按用户授权在前台删除；应用重启后队列运行态计数为 0、历史为空。由于应用前台删除只移除 Room 历史，辅助命令仅删除已核验的 `files/gui-downloads/task-1783691348285-1`，删除后目录只剩根目录。当前截图仍是 Computer Use 会话内可见画面，未形成新的落盘截图包；因此本节补充真实队列和清理观察，但不宣称 M11 通过。现有落盘视觉包仍需结合本轮可保存的五页截图重新完成基准图对照。

## 2026-07-10 当前五页空态与设置区域辅助截图

本轮 Computer Use 在可见 API37 模拟器中逐页切换并观察下载、格式、队列、历史、设置；另滚动设置页复核外观与颜色区域。可见结果如下：

- 下载页：地址输入、分析按钮、下载模式、授权确认和禁用的开始下载状态均完整可见。
- 格式页：未分析时保持禁用提示，不伪造可下载格式；底部导航不遮挡内容。
- 队列页：真实任务计数均为 `0`，等待真实下载提示、阶段骨架和输出信息卡均可见。
- 历史页：历史为空提示、搜索框、筛选按钮和类型切换均可见。
- 设置页：顶部设置项和滚动后的 `外观与颜色`、`基准图配色`、`Codex 风格`、颜色方案及关于区域均位于底部导航上方。

为便于后续逐像素复核，保存了设备 `screencap` 辅助截图及同页 UIAutomator XML：`37-download-current-aux-adb.png/.xml`、`38-format-current-aux-adb.png/.xml`、`39-queue-empty-current-aux-adb.png/.xml`、`40-history-empty-current-aux-adb.png/.xml`、`41-settings-top-current-aux-adb.png/.xml`、`42-settings-appearance-current-aux-adb.png/.xml`。本轮已校验六张 PNG 的标准签名并逐个解析 XML；这些文件只记录同一时刻的设备画面，不能替代 Computer Use 的前台可见操作证据；本轮仍未新增真实进行中队列落盘截图，因此 M11 继续未通过。
