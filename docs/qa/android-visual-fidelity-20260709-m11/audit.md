...
历史记录按用户授权在前台删除；应用重启后队列运行态计数为 0、历史为空。由于应用前台删除只移除 Room 历史，辅助命令仅删除已核验的 `files/gui-downloads/task-1783691348285-1`，删除后目录只剩根目录。Computer Use 的前台观察是主证据；不为留档重复真实下载。
