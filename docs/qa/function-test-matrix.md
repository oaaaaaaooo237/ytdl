...
| Batch real URLs | Multiple pasted URLs can be analyzed, queued with concurrency 2, downloaded, and recorded in history | `scripts/real_batch_smoke.ps1`: 3 real URLs, `queue_rows=3`, `queue_statuses=已完成,已完成,已完成`, `history_count=3`, `downloaded_files=3`, `downloaded_bytes=11,007,992` | Pass |
...
| Visual diff | Strict pixel comparison with reference crops | `scripts/visual_compare.py`, `docs/qa/visual-diff.json`; latest exact: Download `1.1145%`, Formats `2.2982%`, Queue `1.1086%` | Not pass |
