...
| History output path and size | Finished download history records the real file emitted by yt-dlp, not the output template, and records the real output file size | `tests/test_download_progress.py::test_finished_download_history_uses_actual_output_path`; `tests/test_workers.py::test_download_worker_emits_actual_output_path_from_ytdlp_print`; `scripts/real_url_smoke.ps1 https://www.youtube.com/watch?v=KYDPpt3eqaQ` printed an existing `history_output_path` and matching `history_file_size_bytes=23,366,190` under `.qa-real-smoke/basic/20260603-042739` | Pass |
...
| Visual diff | Strict pixel comparison with reference crops | `scripts/visual_compare.py`, `docs/qa/visual-diff.json`; latest exact: Download `1.0557%`, Formats `1.7575%`, Queue `2.0336%` | Not pass |
