...
Primary test URL: `https://www.youtube.com/watch?v=KYDPpt3eqaQ`
...
| --- | --- | --- | --- |
| Analysis | Analyze real video URL without blocking startup | `scripts/real_download_matrix.ps1 https://www.youtube.com/watch?v=KYDPpt3eqaQ` analyzed the primary test URL in three modes; selected formats `18`, `140`, `160` | Pass |
| Download modes | Real download for audio+video, audio-only, and video-only | `scripts/real_download_matrix.ps1`: `audio_video_360p` 74,327,155 bytes; `audio_only_128k` 23,366,190 bytes; `video_only_144p` 15,240,017 bytes under `.qa-real-smoke/matrix/20260603-073146` | Pass |
...
| Format preferences | Resolution, container, codec, and audio bitrate controls affect selected format | `tests/test_format_preferences_ui.py`; real KYDP matrix selected `18` for 360p mp4, `140` for 128k-near audio, `160` for 144p H.264 video-only | Pass |
...
| Queue | Progress reaches completed state | `scripts/real_download_matrix.ps1`, all matrix cases `queue_status=已完成`; `scripts/real_url_smoke.ps1 https://www.youtube.com/watch?v=svoD582Pas4` completed under `.qa-real-smoke/basic/20260603-224046` with `queue_status=已完成` | Pass |
| Queue feedback | Starting a download immediately shows task-added feedback and opens the queue | `tests/test_download_progress.py::test_start_download_announces_queued_task_and_opens_queue`; `scripts/real_url_smoke.ps1 https://www.youtube.com/watch?v=svoD582Pas4` printed `status=已添加 1 个下载任务，正在队列页显示进度。` | Pass |
...
| History output path and size | Finished download history records the real file emitted by yt-dlp, not the output template, and records the real output file size | `tests/test_download_progress.py::test_finished_download_history_uses_actual_output_path`; `tests/test_workers.py::test_download_worker_emits_actual_output_path_from_ytdlp_print`; `scripts/real_url_smoke.ps1 https://www.youtube.com/watch?v=svoD582Pas4` printed an existing `history_output_path` and matching `history_file_size_bytes=1,294,911` under `.qa-real-smoke/basic/20260603-224046` | Pass |
| Batch real URLs | Multiple pasted URLs can be analyzed, queued with concurrency 2, downloaded, and recorded in history | `scripts/real_batch_smoke.ps1` latest run under `.qa-real-smoke/batch/20260603-073249`: 3 real URLs, `queue_rows=3`, `queue_statuses=已完成,已完成,已完成`, `history_count=3`, `downloaded_files=3`, `downloaded_bytes=11,007,992`, history output paths match record titles and each `file_size_bytes` matches the real file | Pass |
...
| Visual diff | Strict pixel comparison with reference crops | `scripts/visual_compare.py`, `docs/qa/visual-diff.json`; latest exact: Download `1.3239%`, Formats `1.7575%`, Queue `2.0914%` | Not pass |
...
| Preview | Play-while-downloading obtains stream and failure does not block download | `scripts/real_download_matrix.ps1`, `audio_video_360p` loaded a real preview URL while real download completed; `scripts/real_full_smoke.ps1` loaded a preview URL under `.qa-real-smoke/full/20260603-073326` | Pass |
| Subtitles | Real subtitle-file download and ffmpeg burn-in preserve source files and record burned output | `scripts/real_subtitle_smoke.ps1` with `https://www.youtube.com/shorts/oXFad1nt6v0` under `.qa-real-smoke/subtitles/20260603-073355`: subtitle file case `subtitle_files=2`; burn case output `.burned.mp4`, `media_files=2`, `downloaded_bytes=10,634,514` | Pass |
