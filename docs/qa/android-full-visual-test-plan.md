...
- 2026-07-06 本轮继续用 Computer Use 跑真实 `tkxzMEfp49Q` 链路，前台观察到 1080p 视频+音频分离下载、阶段化进度、原生合并、历史完成记录、系统导出写出到 `/sdcard/Download`、历史打开播放。边界：URL 输入时出现 Android 输入法浮层，所以该次仍不能作为最终 T12 干净输入验收；确认删除和真实 cookies 选择仍未完成。

剩余验收：确认删除需用户明确授权后补前台验证；真实 cookies 文件选择仍需用户提供测试文件；最终 T12 还需要一次不出现输入法浮层的干净 URL 输入复测。
...
当前状态：未完成。2026-07-05 已恢复 Computer Use 并完成冷启动真实前台流程和一轮视觉密度截图修复；同日已补强队列页取消、系统通知栏取消和通知权限拒绝时 app 内进度仍可见的 connected 真实链路，覆盖早取消 race、通知展开后点击 `取消` action、拒权后队列阶段条可见，以及 Room `canceled` 历史写入。2026-07-06 已再次用 Computer Use 前台复测真实 `tkxzMEfp49Q` 下载链路，覆盖 1080p 视频+音频分离下载、阶段化进度、原生合并、历史完成记录、系统导出写出和历史打开播放。T12 仍未通过，因为 URL 输入时出现 Android 输入法浮层，确认删除和真实 cookies 文件选择也仍未完成。
...
2026-07-06 更新：Computer Use 能激活并操作 `Android Emulator - ytdl_api37_play_x86_64:5554`。五页前台导航截图已落盘到 `docs/qa/android-computer-use-20260706/`；同日真实下载复测已观察到阶段进度、历史完成记录、系统导出文件和播放画面。该证据已补齐系统导出写出和打开播放的前台留痕，但仍不能写成最终 T12 通过，原因是干净 URL 输入、确认删除和真实 cookies 文件选择尚未完成。
...
| T12 全量 MVP 回归 | 未完成 | `testDebugUnitTest`、`assembleDebug`、API37 connected `YtdlAppUiTest` 8/8 已覆盖普通下载、Shorts、队列取消、通知栏取消和通知拒权 app 内进度；Computer Use 已完成冷启动真实前台流程、五页导航截图、真实下载、阶段进度、历史、系统导出写出和打开播放；但最终干净 URL 输入、确认删除、真实 cookies 文件选择和失败恢复前台路径仍未完成 |
...
6. M9 前置：已补外观配色设置持久化与即时应用、队列/通知取消、历史打开/分享/导出/删除、字幕独立文件选择绑定、重复下载唯一输出和历史相对 URI；全量单测、构建、connected 已通过。2026-07-05 Computer Use 已恢复并完成普通视频与 Shorts 的部分真实前台流程；同日队列页取消补强了早取消 race 和 Room `canceled` 历史写入，系统通知栏展开后取消也已有 connected 辅助证据。2026-07-06 已补前台真实下载、导出写出和打开播放证据，但仍待最终干净输入全量验收。
7. M9/T12：继续完成 Computer Use 前台可见全真全量验收，重点补齐不触发输入法浮层的 URL 输入、确认删除、真实 cookies 文件选择和一个可恢复失败场景；2026-07-05 已完成一轮视觉密度截图修复，但这仍不是模拟器最终通过口径。
