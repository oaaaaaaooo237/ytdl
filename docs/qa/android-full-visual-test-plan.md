...
6. 对 `tkxzMEfp49Q` 的后续测试顺序必须是：真实分析 -> 分离视频/音频下载能力 -> MediaProcessor 合并能力 -> GUI 绑定 -> 前台可视下载/合并/队列/历史/导出/通知全流程。
...
- 没有 MediaProcessor 合并能力前，不再反复测试 `需 ffmpeg 合并` 的 GUI 标签。
- 没有 Android ffmpeg 字幕嵌入/烧录能力前，不再把“视频+音频+字幕”GUI 流程作为已通过功能展示。
...
| 队列页 | 展示视频下载、音频下载、合并、字幕处理或跳过、完成/失败/取消状态，进度来自真实事件 | 不用静态样例队列；不重复内部 track 检查 |
| 历史页 | 完成后出现历史记录，打开/导出/删除至少覆盖一个成功项 | 不重复所有排序/筛选边角，除非本轮改动历史逻辑 |
| 设置页 | 保存位置、cookies 引用、解析器版本、ffmpeg 能力、通知权限、隐私/授权说明、外观配色状态 | 不读取或展示 cookies 内容 |
| 前台服务/通知 | 下载过程中用户可感知，通知拒绝时 app 内仍显示状态 | 不把系统通知截图作为唯一证据 |
| 隐私与失败 | 至少覆盖空 URL 或非法 URL、ffmpeg 缺失/处理失败中的一个真实错误展示；日志/界面不泄露敏感内容 | 不在最终可视主路径里故意触发所有错误；其余由 T10/T11 测试覆盖 |
| Shorts 兼容 | 用 `https://www.youtube.com/shorts/QBwpO9f0oAw` 做分析和短下载抽样 | 不重复主路径的历史/设置/导出全套断言 |

通过 T12 前，以下能力必须已经有新鲜证据：T1 构建与单元测试、T2 真实分析、T8C 指定格式分离下载、T8D required URL 核心合并、T8E Android ffmpeg 字幕处理、T6/T7 GUI 绑定、T9/T10/T11 历史/隐私/失败恢复。
...
本轮辅助证据：2026-06-20 已用前台 UIAutomator 流程输入 `https://www.youtube.com/watch?v=tkxzMEfp49Q`，点击 `分析`，页面显示真实标题 `Jalen Brunson 'Captain Clutch' Moments in Knicks Championship Season`、真实缩略图、时长 `08:02`、格式摘要 `1080p MP4 需 ffmpeg 合并` 和 `分析完成，可以开始下载。`

截图：`docs/qa/android-visible-real-flow/03-analysis.png`

Shorts 辅助证据：2026-06-20 已用前台 UIAutomator 流程输入 `https://www.youtube.com/shorts/QBwpO9f0oAw`，点击 `分析`，页面显示真实标题片段 `Luka and Jalen`、真实缩略图、时长 `00:14`、格式摘要 `1280p MP4 需 ffmpeg 合并` 和 `分析完成，可以开始下载。`

截图：`docs/qa/android-visible-real-flow/shorts-03-analysis.png`

Shorts Computer Use 历史证据：2026-06-20 已用 Computer Use 在前台可见 Android Emulator 窗口内点击输入框、使用模拟器软键盘候选输入、点击 `分析`，页面显示真实标题片段 `Luka and Jalen`、真实缩略图、时长 `00:14`、格式摘要 `1280p MP4 需 ffmpeg 合并`。由于输入方式不符合 2026-06-20 新增的禁用软键盘规则，本证据只能证明分析结果展示，不计入最终前台输入验收。

截图：`docs/qa/android-visible-real-flow/cu-shorts-analysis-complete.png`

普通视频 Computer Use 历史证据：2026-06-20 已用 Computer Use 在前台可见 Android Emulator 窗口内清空输入框、使用模拟器软键盘候选输入 `https://youtu.be/tkxzMEfp49Q`、点击 `分析`，页面显示真实标题 `Jalen Brunson 'Captain Clutch' Moments in Knicks Championship Season`、真实缩略图、时长 `08:02`、格式摘要 `1080p MP4 需 ffmpeg 合并`，并显示 `分析完成` 状态。由于输入方式不符合 2026-06-20 新增的禁用软键盘规则，本证据只能证明分析结果展示，不计入最终前台输入验收。
...
- 如果下载页或格式页显示“需 ffmpeg 合并”，必须另走 T8 合并测试，不能用本项代替。
...
- 支持项可选，不支持项不可选或置灰。
- 高分辨率音视频分离时明确标记需要 ffmpeg 合并。
- 实际下载摘要与用户选择一致。
- 选择“视频+音频”且目标分辨率只有分离视频流时，必须把下载任务路由到 T8 的 ffmpeg 合并链路。
...
### T8 MediaProcessor 与 ffmpeg 媒体处理测试

目的：先证明第一阶段最核心需求“分离视频流 + 音频流 -> 合并输出文件”真实可用，并在 GUI 绑定前补齐 Android ffmpeg 字幕嵌入/烧录能力。

执行时机：现在优先执行。没有 T8B/T8C/T8D 通过前，不再把高分辨率 `视频+音频` GUI 流程标记为可验收；没有 T8E 通过前，不再把 `视频+音频+字幕` 或第一阶段 ffmpeg 字幕能力标记为可验收。

#### T8A MediaProcessor 合同与路线

测试项：

- 文档 `docs/android-media-processor.md` 记录当前事实：仓库没有 Android ffmpeg，Windows `ffmpeg.exe` 不能用于 Android，Gradle 缓存没有现成 ffmpeg-kit。
- 代码定义 `MediaMergeRequest`、`MediaProcessingResult`、`MediaProcessor`。
- 单元测试证明：原生 muxer 只承诺兼容流的音视频合并，不承诺字幕烧录；字幕烧录必须路由到后续 ffmpeg processor。

通过标准：

- `:app:testDebugUnitTest` 通过。
- `:app:assembleDebug` 通过。
- 文档明确原生 MediaMuxer 与 ffmpeg 的边界。
...
- 单元测试覆盖：video/audio 输入必须 distinct 且可读，outputFile 必须在受控输出根目录内且不能等于输入文件，原生 muxer 不支持字幕嵌入/烧录并返回 ffmpeg 路由的 `UnsupportedOperationException`。
...
- 说明：本项只证明 Android APK 内置 Chaquopy/yt-dlp 可按明确 format id 下载 video-only 与 audio-only 两个分离文件；仍不是 GUI 验收，也不包含合并、ffmpeg、历史或通知。
...
#### T8E Android ffmpeg 字幕嵌入/烧录

测试项：

- 采用项目自编译最小 LGPL FFmpeg 8.1.x 动态库路线；不得把 FFmpegKit/mobile-ffmpeg/bihe0832 AAR 试验当作通过证据。
- 记录源码版本、来源 URL、签名或 hash、configure flags、启用库、许可证判断、ABI 和包体积。
- `ffmpeg -version` 或等价 JNI bridge 版本探针可在 API37 x86_64 16KB 模拟器运行。
- 字幕文件真实下载。
- 字幕嵌入真实生成输出文件。
- 字幕烧录真实生成输出文件。
- 每个打包 `.so` 有 ELF LOAD 16KB alignment 证据；AAB/APK 有 16KB page-size 检查证据。

通过标准：

- 每个输出文件存在且可识别。
- 字幕嵌入输出中可识别字幕流。
- 字幕烧录输出中可识别视频/音频轨，显示不依赖外置字幕流。
- 许可证、ABI、16KB、包体积、源码获取说明记录完整。
- 该项未通过前，不得声称“第一阶段 ffmpeg 字幕能力完成”。

当前状态：未完成。

当前证据与下一步：

- 2026-06-20 在 API37 模拟器执行 `which ffmpeg || echo no-ffmpeg`，结果为 `no-ffmpeg`。
- 当前 Android 工程尚未包含 `android/app/src/main/jniLibs/x86_64/ffmpeg` 或 `arm64-v8a/ffmpeg`。
- 2026-06-20 审计已拒绝 `mobile-ffmpeg` / `bihe0832` AAR 作为主线方案。
- 下一步按 M5a-M5e 执行：先清理错误依赖并建立 FFmpeg 构建环境门禁，再自编译 LGPL FFmpeg、接 JNI/命令桥、跑字幕嵌入/烧录 instrumentation，最后补许可证/16KB/体积/arm64 证据。
- 因此目前不能声称 Android ffmpeg、字幕嵌入或字幕烧录已通过；T8E 通过后才进入 M6/M7 的下载编排、GUI 绑定和可视验收。
...
6. 开始下载。
7. 观察队列真实显示视频流下载、音频流下载、MediaProcessor 合并、字幕处理或明确跳过原因、完成状态。
8. 完成后检查历史。
9. 导出或打开合并后的输出文件。
10. 检查设置页、cookies 边界、MediaProcessor/ffmpeg 状态、通知权限和隐私说明。
...
| T5 前台 GUI 单文件下载进度 | 暂缓 | 先完成 T8C/T8D/T8E，再做 GUI 绑定和可视验收 |
| T6 格式真实可选项 | 部分实现/待复测 | 已有格式选择模型雏形；待 T8C/T8D/T8E 完成后重测 GUI 绑定 |
| T7 前台服务通知 | 未完成 | 尚未实现 |
| T8A MediaProcessor 合同与路线 | 已完成合同层 | 已定义合同、校验边界和原生 muxer/ffmpeg 职责；真实合并在 T8B |
...
| T8E Android ffmpeg 字幕嵌入/烧录 | 未完成 | GUI 绑定前必须补齐 ffmpeg 字幕能力 |
...
2. M5/T8E：补 Android ffmpeg 字幕嵌入/烧录能力，记录许可证、ABI 和包体积影响。
3. M6：实现下载编排、前台服务、通知和真实队列状态，禁止旧 360p/direct 假回退。
4. M7：把真实分析、格式选择、分离下载、合并、ffmpeg、队列状态绑定到五页 GUI。
