...
2. Android 功能验收必须使用 Computer Use 操作前台可见模拟器窗口并完成真实操作观察。UIAutomator 只能作为辅助定位/回归证据，不能记为通过。
...
8. 每项测试完成后必须记录命令、设备、时间、输出摘要、是否通过、遗留问题。
9. 前台可视测试输入 URL 必须拟真真机：用 Computer Use 点击输入框后，允许并优先使用 Android 系统软键盘完成输入。不得通过候选词替换、自动补全、手写浮层、Gboard 菜单、后台 adb/脚本直接写入等方式替代用户可见输入；必须在前台观察确认完整 URL 已真实进入输入框且未被改写。只有软键盘遮挡关键控件、改写输入或导致流程无法继续时，才记录为测试阻断。剪贴板、硬件键和可访问性写入只能作为非验收辅助或明确标注的环境诊断证据，不能替代最终前台验收。
...
辅助入口：`scripts/android_real_smoke.ps1` 默认只生成环境、单测、打包和无真实 YouTube 请求的 connected 安全集证据；它不能替代 Computer Use 前台可视验收。需要真实分析或下载时，只能显式打开对应开关，并继续遵守本节 429 节流规则。真实字幕下载暂停期间，脚本会拒绝 `-RunRealSubtitleDownload`，除非用户明确恢复后额外传入 `-AllowRealSubtitleDownload`。
...
| Shorts 兼容 | 用 `https://www.youtube.com/shorts/oXFad1nt6v0` 做分析和短下载抽样 | 不重复主路径的历史/设置/导出全套断言 |
...
- 使用当前普通视频主路径 `https://www.youtube.com/watch?v=PqQNXB6hhUs`。
...
4. 输入 `https://www.youtube.com/watch?v=PqQNXB6hhUs`。
5. 点击 `分析`。
6. 等待真实分析完成。
...
当前状态：M6 核心/声明层已实现，后续 M9/T12 已以前台可见方式复核系统通知允许路径和通知拒权设置页可见状态。
...
- 通知允许状态下已在 Computer Use 前台可见 API37 模拟器中下拉、展开并从通知栏点击 `取消`；拒绝通知权限后设置页可见状态已在 M9.14/M9.17 前台复核。
- GUI `开始下载` 按钮已通过 `DownloadCoordinator.startForegroundDownload()` 接入真实前台服务，不再调用旧 `downloadSingleFile()`；五页完整绑定、历史/导出和取消按钮已在 M9/T12 形成前台证据。后续 M11 只处理视觉一致性，不重新定义功能验收。
...
当前状态：通知允许状态下的系统通知可见、展开和通知栏取消已完成 Computer Use 前台验证；通知拒权后 app 内进度可见已有 connected 辅助证据，设置页拒权状态也已在 M9.14/M9.17 用 Computer Use 前台复核。
...
当前状态：M8 单元层已实现；主要前台恢复路径已在 M9.x 与 M9.17 汇总到 T12。真实字幕下载恢复专项、更多罕见系统/网络故障和真机特有失败仍留到后续专项或 M10。
...
后续边界：T12 已覆盖默认无字幕 coherent 主路径；更多罕见失败恢复、真实字幕下载恢复专项和真机特有故障不作为当前 T12 阻塞项，后续按独立任务验证。
...
1. 启动模拟器。
2. 安装当前 APK。
3. 用 Computer Use 在前台可见模拟器窗口点击 URL 输入框，弹出 Android 系统软键盘，并通过软键盘输入 `https://www.youtube.com/watch?v=PqQNXB6hhUs`；确认输入未被候选词、自动补全或手写浮层改写。
4. 分析。
5. 选择真实支持的高分辨率 `视频+音频` 选项，且该选项需要分离视频流和音频流。
6. 开始下载。
7. 观察队列真实显示视频流下载、音频流下载、MediaProcessor 合并和完成状态；暂停字幕测试期间不选择字幕，未选择字幕时不得出现 `字幕文件` 阶段。
8. 完成后检查历史。
9. 导出或打开合并后的输出文件。
10. 检查设置页、cookies 边界、MediaProcessor/MVP2 字幕能力说明、通知权限和隐私说明。
11. 触发一个可恢复失败场景并确认中文错误可读且不泄露敏感信息。
12. 用 `https://www.youtube.com/shorts/oXFad1nt6v0` 做一次 Shorts 分析和短下载抽样。
...
2026-07-09 历史状态（已由 2026-07-10 地址集替代）：API37 模拟器 M9/T12 已通过。M9.17 release gate 曾用 Computer Use 在前台可见 API37 模拟器中完成当时主地址 `https://youtu.be/lcFR2mFSmSs?si=FqJ3ZTdKRq6NAt6G` 的系统软键盘输入、真实分析、格式页 MP4 原生合并兼容复核、无字幕视频/音频分离下载、原生合并、队列完成、历史落地和系统播放器打开；同日也补齐当时 Shorts 样本、通知拒权设置页可见状态、Gboard 直接弹出输入环境、格式兼容修复证据，以及新鲜单元测试、debug 打包和环境脚本验证。真实字幕下载当前按用户要求暂停，不再作为默认 T12 阻塞项。T12 通过不等于整个 Android MVP 目标完成；M11 视觉一致性、M10 真机验收和 Play 发布交付项仍需后续推进。
...
| T2 真实 URL 分析核心 | 旧地址历史已测/新地址待节流复测 | API37 历史证据为 `tkxzMEfp49Q`；后续默认改用 `PqQNXB6hhUs`，真实 connected 网络测试需显式 `realYoutube=true` |
...
| T7 前台服务通知 | 通知允许前台已测/拒权前台状态已测 | M6 已完成 foreground service 声明、通知控制器、`DownloadService` 承载 pipeline 和核心状态模型；M9 前置补上队列页真实取消入口和通知取消 action；2026-07-05 队列页取消、系统通知栏展开后取消、通知拒权后 app 内队列阶段条可见均通过 API37 connected 真实链路，早取消 race 已修复；M9.7 已用 Computer Use 前台可见窗口证明通知允许状态下的 `YTDL 下载任务` 可见、可展开、可从通知栏取消；M9.14/M9.17 已用 Computer Use 前台复核通知拒权设置页状态 |
...
| T8D required URL 核心合并 smoke | 旧地址能力层通过/新地址待节流复测 | API37 已用 `tkxzMEfp49Q` 完成分析、format 160/139 分离下载、原生合并和 track 检查；后续默认改用 `PqQNXB6hhUs` |
| T8E 独立字幕文件输出 | 能力层通过/历史导出绑定层已补/合成字幕历史前台已测/真实字幕下载暂停 | API37 已下载 `tkxzMEfp49Q` 自动英文 `vtt` 字幕到 app 私有 cache；M9 前置补上格式页字幕开关和下载请求 `selectedSubtitles` 传递；M9.8 已补历史 `subtitleOutputUris`、Room v3 迁移、队列/历史“媒体文件 + 独立字幕文件”元信息和“分享字幕/导出字幕”动作测试；前台可见窗口已用合成小型字幕历史记录点击“分享字幕”“导出字幕”，系统分享面板和保存器均可打开且未显示私有路径；真实字幕下载当前暂停，不作为默认 T12 阻塞项 |
...
| T11 失败恢复 | 主要前台恢复路径已测/罕见专项待后续 | 失败文案和脱敏已覆盖；2026-07-05 冷启动流已验证空 URL 前台提示；2026-07-06 最新 APK 已用 Computer Use 前台点击系统软键盘输入非法文本 `avx`，显示中文错误且队列无真实任务；M9.5 已用合成缺失输出历史记录验证 `打开` / `分享` / `导出` 的历史页可见恢复提示；M9.6 已用合成小文件验证系统导出保存器取消后的历史页可见恢复提示；M9.7 已用真实任务验证通知栏取消后通知和 app 内状态均进入取消；通知拒权后 app 内进度可见已有 connected 真实链路且设置页拒权状态已前台复核；2026-07-06 已覆盖音频段 `HTTP Error 403` 网络失败后只重试失败段并最终成功；真实字幕下载恢复、更多网络/系统罕见故障和真机特有失败留到后续专项 |
| T12 全量 MVP 回归 | API37 模拟器前台通过 | 2026-07-09 M9.17 已汇总新主地址完整默认无字幕路径、当前 Shorts 样本、通知拒权可见状态、Gboard 直接弹出输入环境、格式兼容修复证据、队列/历史/打开链路，以及新鲜 `testDebugUnitTest`、`assembleDebug`、环境脚本和 `git diff --check` 证据。真实字幕下载继续暂停；M10 真机、M11 视觉一致性和 Play 发布交付项不属于本项通过结论 |
...
7. M9/T12：已完成 API37 模拟器 Computer Use 前台 release gate。后续不得回退到后台写入、硬件键、剪贴板输入或字幕探针；真实字幕下载按用户要求继续暂停。
8. M11：先在模拟器和截图层面继续做 `docs/android-gui-reference-v3.png` 的五页视觉一致性与 Codex 外观设置审计/修复，避免把“可运行”误认为“已尽可能还原基准图”。
9. M10：等真机已连接时，在小米14或同级 `arm64-v8a` 真机上验证通知、后台下载、打开/导出/分享、存储拒权恢复和真实 UI 适配；该阶段只做真机验收，不做正式 Google Play 商店交付。
10. Google Play 商店交付：发布签名、商店列表、最终 Data safety 提交、隐私政策 URL 和商店截图仍未开始，需单独任务和证据。

## 2026-07-09 M11 视觉一致性审计启动

本轮开始执行 Continuation Task M11。已用 Computer Use 在前台可见 API37 模拟器窗口检查当前 APK 的五页 `基准图配色`，并通过滚动设置页后切换到 `Codex 风格` 进行前台观察。
...
```text
docs/qa/android-visual-fidelity-20260709-m11/audit.md
```
...
本节是 M11 的视觉审计启动记录，不代表 M11 已通过。M11 通过仍需要完成 scoped UI 修复、focused 测试、`testDebugUnitTest`、`assembleDebug`、Computer Use 前台五页复核、基准/Codex 配色复核、真实进行中队列的前台观察，以及剩余偏差说明。
...
2026-07-10 M11 恢复诊断：环境脚本确认 API37 和 Gboard 软键盘前置；Computer Use 曾通过模拟器 `Raise` 动作恢复可见前台控制，前台切换五页并滚动设置页确认外观/配色控件不受底栏遮挡。地址框点击后完整 Gboard 从底部弹出，但开始逐键输入时 Windows 再次拒绝窗口激活；后续辅助 Activity 状态为 YTDL 顶层，Computer Use 捕获却连续显示黑色桌面/天气。该状态下未继续发送业务输入，也未使用 adb/剪贴板/硬件键代替 URL 输入；未触发任何 YouTube 请求。M11 后续只需完成前台五页复核和真实进行中队列观察。
...
2026-07-10 最新补充：Computer Use 后续恢复到可见 YTDL 窗口并观察到一条历史链接的真实视频+音频任务进入原生合并，但因模拟器 `/data` 当时仅余约 `215 MB` 而失败。已修复合并前容量检查及失败中间流清理，并已在前台删除历史记录、辅助核验下载目录为空。后续真实前台测试仅用 `PqQNXB6hhUs`、`svoD582Pas4` 或 Shorts `oXFad1nt6v0`，每次结束都要删除对应记录并核验无测试文件残留。M11 后续只需完成前台真实进行中队列观察和五页视觉复核。

2026-07-10 当前主地址真实流程补充：Computer Use 在可见 API37 模拟器中用完整底部 Gboard 逐键输入 `https://www.youtube.com/watch?v=PqQNXB6hhUs`，辅助 UIAutomator 树核验完整字段；真实分析成功并显示 `09:00`、真实缩略图和 `1080p MP4 需原生合并`。格式页显示当前实际 `1080p/720p/480p/360p/240p/144p`，字幕不可选且未选择。无字幕视频+音频下载的前台队列显示 `39.2 MB / 187.9 MB`、`6%`、随后 `107.1 MB / 187.9 MB`、`19%`，最终完成 `196.3 MB / 196.3 MB`、视频/音频/原生合并三阶段勾选、绿色完成和右下 `1080p`。前台删除只移除 Room 历史记录；按用户授权，辅助命令删除已核验的 `files/gui-downloads/task-1783691348285-1`，删除后目录核验只剩根目录。重启应用后队列计数为 0、历史为空，这是运行态 UI 复核，不作为文件删除原因的证明。Computer Use 前台观察是主证据。
