...
- 选择并记录 Android ffmpeg 打包路线、许可证、ABI 和包体积。
- `ffmpeg -version` 可在 x86_64 模拟器运行。
- 字幕文件真实下载。
- 字幕嵌入真实生成输出文件。
- 字幕烧录真实生成输出文件。
...
- 因此目前不能声称 Android ffmpeg、字幕嵌入或字幕烧录已通过；先实现 T8A-T8D 的核心分离流合并，再补 T8E，之后才进入 GUI 绑定和可视验收。
