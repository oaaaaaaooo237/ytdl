...
7. M9/T12：继续完成 Computer Use 前台可见全真全量验收，重点补齐确认删除、真实 cookies 文件选择和更多可恢复失败场景；全流程输入必须按真机拟真口径使用系统软键盘，并确认 URL 未被候选词、自动补全或手写浮层改写。2026-07-05 已完成一轮视觉密度截图修复，但这仍不是模拟器最终通过口径。
