...
- 字幕文件与同一任务的合并后视频音频文件建立关联。
- 历史和导出路径能显示/处理两个输出：合并媒体文件和字幕文件。
...
当前证据与下一步：MVP1 方向已调整为独立字幕文件输出；下一步实现字幕分析、下载、队列/历史/导出绑定。FFmpeg 字幕嵌入/烧录移入 MVP2，不阻塞 T12。
...
| T8E 独立字幕文件输出 | 未完成 | MVP1 要输出独立字幕文件并关联历史/导出；字幕嵌入/烧录为 MVP2 |
