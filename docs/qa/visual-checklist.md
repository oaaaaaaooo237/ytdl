...
- Real download matrix completed audio+video `18`, audio-only `251`, and video-only `160`; each case wrote history and downloaded a real file.
- Real batch smoke completed 3 URLs including a Shorts link with concurrency 2.
- Real subtitle smoke completed subtitle-file download and ffmpeg burn-in on the Shorts link.
...
- Current UI has been moved toward the selected reference: custom title bar, compact icon+label left rail, bottom yt-dlp status bar, light Win11 surfaces, teal primary actions, staged download flow, format segments, thumbnail queue cards, and queue-side recent history.
- Strict visual comparison is generated by `scripts/visual_compare.py` into `docs/qa/visual-diff.json`.
- Latest strict comparison: Download exact `1.1145%`, Formats exact `2.2982%`, Queue exact `1.1086%`.
