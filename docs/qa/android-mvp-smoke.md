...
## 2026-07-14 重试纠偏与字幕前台隐藏验收口径

- 失败队列的 `重试`、失败历史的 `再次下载` 必须重新分析原 URL，分析成功后停在格式页，不得自动开始下载。
- 上次模式、分辨率和编码对应的精确格式仍存在时应自动恢复；原格式失效时只使用本次分析结果中的兼容默认项，由用户确认后下载。
- 历史重试只在加密最小草稿可成功解密时可用；草稿只包含 URL 和上次路由。损坏、旧版本或已随历史删除的草稿不得显示重试入口。
- 格式、队列、历史和设置说明不显示字幕选项、字幕阶段、字幕产物或字幕动作；GUI 发起的下载请求默认且固定不选择字幕。后端字幕能力保留，但不属于本版前台验收范围。
- 自动化必须覆盖重新分析而非直接下载、旧格式恢复/兼容回落、加密草稿删除与损坏拒绝、字幕入口不存在；随后顺序运行全量单测与 debug 构建。
- Computer Use 在可见 API37 模拟器中复核上述前台行为。可使用现有失败记录，避免为验证重试重复真实下载；本轮不测试字幕。
...
### 2026-07-14 保存链路与界面优化续作

本节覆盖并替代此前仍把“导出”列为后续验收项的当前口径；历史验收记录不改写。

- 用户选择系统文件夹时，下载和合并可以在 App 私有任务目录中中转。外部产出全部写入成功后，历史只记录对应 `content://` 地址，并立即删除该私有任务目录；外部保存失败时保留私有源文件供失败恢复。
- 使用默认 App 私有目录时，单独视频、单独音频、独立字幕或合并成品都是用户完成文件，必须保留。视频和音频中间流在合并成功后立即删除。
- “清理临时缓存”只删除 App 私有下载根目录内、未被队列或历史引用的失联过程文件。不得删除完成文件、合并文件、字幕、外部用户文件、数据库或程序资源；活动下载时继续禁止清理。
- 历史页取消手动“导出/导出字幕”。打开和分享同时支持 App 私有文件与外部 `content://` 文件；系统文件夹保存仍由下载完成链路自动执行。
- 地址栏改为原生 `EditText` 1 至 5 行自适应换行，保留系统软键盘及原生长按全选、复制、粘贴、删除能力。
- 启动器采用用户选择的 C 版 3D 配色，改为无白色异形底、四边铺满的正方形图标。

本轮聚焦回归覆盖保存 URI、外部成功清理、私有成品保护、合并后流文件清理、失联缓存清理、历史打开/分享、导出入口移除、地址栏行数和图标边缘。存储行为以代码和自动化边界测试为主要验证，不为该行为重复真实网站下载。

地址输入统一纪律（模拟器和真机均适用）：输入前先观察并记下当前字母键盘、符号键盘的进入方法和目标符号位置；每个可见按键点击之间至少间隔 `0.18 秒`；切换键盘页面后至少等待 `0.5 秒` 再点击下一键。完整地址必须一次性输入完毕，全部按键结束后再统一核对字段；核对准确前不得点击“分析”。

### 2026-07-14 保存生命周期与优化最终复核

- 独立只读审计先后发现并修复：历史删除后完成媒体可能被缓存清理误删、SAF/合并清理失败未向上反映、旧导出路径残留，以及任务目录符号链接越界和部分删除失败仍误报成功。复审后，符号链接只删除链接节点、递归删除限制在受控任务目录内；缓存仍有残留时返回“部分临时文件未能清理”并保留可重试统计。所有本轮审计子任务均已关闭。
- 最终自动验证：`:app:testDebugUnitTest --no-parallel` 的 31 个套件共 `310/310` 通过，0 失败、0 错误、0 跳过；`:app:assembleDebug --no-parallel` 通过。Gradle 任务顺序执行。
- Computer Use 前台复核：规定的 `node_repl` 最小 smoke 与 `sky.list_apps()` 均成功，只操作唯一可见 API37 模拟器。启动器中的 C 版图标使用自适应图标外层填色，主体铺满系统图标区域，不再出现白色异形托底。
- 地址栏复核严格遵守统一输入纪律：先观察字母键位、两页符号键盘入口和符号位置，再以每键至少 `0.18 秒`、每次页面切换至少等待 `0.5 秒` 的节奏，一次输入 `https://example.com/abcdefghijklmnopqrstuvwx?x=1234567890`；全部输入结束后只统一核对一次。字段逐字准确并自动换为 3 行，未超过 5 行；未点击“分析”，未产生网络请求。
- 历史空态不再显示“导出/导出字幕”；设置页显示 App 私有默认保存位置和“恢复默认路径”，点击保存位置主体可打开 Android 系统目录选择器并可安全返回。点击“清理下载缓存”会先提示只删除未被下载记录引用的 App 私有临时文件，不删除完成文件、合并文件或所选目录文件；本轮只检查并取消确认，没有执行删除。
- 本轮没有真实网站分析、下载或字幕请求。保存生命周期由代码、自动化边界测试和独立审计确认；第 6 步小米 14 最终 Computer Use 验收仍待执行，不能据此宣称 M10/D2 已通过。

### 2026-07-14 格式编码、再次下载与字幕前台隐藏复核

- Computer Use 通过唯一可见 API37 模拟器的 scrcpy 窗口完成前台操作；规定的最小连接 smoke 和应用窗口发现均成功。URL 全部由底部 Gboard 逐键输入，相邻按键至少间隔 `0.22 秒`，键盘页切换至少等待 `0.65 秒`；未使用剪贴板、adb 注入、硬件键或 `type_text` 替代输入。
- 主 YouTube 地址的失败任务在队列显示 `重试`。恢复网络后点击该按钮会重新分析原地址、进入格式页并恢复上次 `720p / H.264`，不会自动开始下载。
- 专项地址 `https://www.eporner.com/video-5czAhpxw6bT/gender-x-date-switch-jade-venus-and-ariel-demure/` 当前只允许“视频下载”，仅音频和视频+音频均灰显；格式页按分辨率各一行，当前提供 `2160p/1440p/1080p/720p/480p/360p/240p`，显式 `2160p / AV1 / 单文件` 进入失败队列后，队列和历史均在右侧显示 `AV1` 与 `2160p`。
- 上述失败历史显示 `再次下载`。点击后实际重新分析原地址并回到格式页，恢复 `2160p / AV1 / 单文件`，仍需用户确认或调整后才会创建新任务。该复核同时确认 Android Keystore 加密重试草稿已可在真机提供器上写入；先前的 `Caller-provided IV not permitted` 已改为由 Keystore 加密器生成 IV。
- 专项地址 `https://www.eporner.com/video-EjpMHywtGhj/luna-colombiana-settles-her-boyfriend-s-gambling-debt-with-a-high-stakes-gamble-of-her-own/` 当前解析最高为 `1080p`，不是 `2160p`；页面只显示当前实际提供的 `1080p/720p/480p/360p/240p`。同分辨率只显示一行，右侧 `AV1/其他` 切换会改变精确格式选择；未知编码不在摘要、队列或历史中猜测为具体编码。
- 格式、队列、历史和设置说明均未出现字幕选项、字幕阶段或字幕产物动作；GUI 下载固定不选择字幕。后端字幕请求模型和管线仍保留，本轮未发起字幕请求。
...
### 2026-07-15 Android 1.0.1 任务页、进度和文件名验收

- 版本：`versionCode=2`、`versionName=1.0.1`。独立“格式”页已合并到下载页，底部导航最终为“下载 / 任务 / 设置”三项，图标和文字均比 1.0.0 更醒目；“任务”页按顺序展示当前任务、等待任务和历史记录。
- 下载进度：可见模拟器中的真实任务显示“速度 3.7 MB/s · 已下载 319.7 MB · 总计 371.3 MB”。速度和已下载大小会更新，同一下载阶段的总大小保持不变。
- 顺序下载：Eporner 单视频下载进行时，第二条 YouTube 任务成功加入等待队列；任务页显示“正在下载 1 · 等待 1”。取消当前任务后，下一条自动成为当前任务，同时始终只有一个任务执行。
- 再次下载：取消的 YouTube 和 Eporner 记录均显示“再次下载”。点击后重新分析原地址并回到下载页，保留仍然有效的上次选择，不会直接开始下载。
- 历史文案：第二行只显示“视频+音频 / 视频 / 仅视频 / 仅音频”、主文件名和时间；`web`、“媒体文件”、重复分辨率和完整路径不再显示。编码和分辨率继续放在右侧徽标中。
- 文件名：直接下载和原生合并后的主文件默认使用解析出的媒体标题，保留正确扩展名。Eporner 完成记录在前台显示 `Gender X Date Switch Jade Venus And Ariel Demure.mp4`；长标题的实际文件名和历史记录都保留扩展名。合并过程中的视频流、音频流放在 `.parts`，不作为成品展示。
- 真实网站：所有地址都通过可见 Gboard 逐键输入，每键至少间隔 `0.18` 秒，键盘页面切换至少等待 `0.5` 秒，核对完整地址后才分析。YouTube 视频+音频完成一次；Eporner 单视频完成一次；Pornhub 成功显示标题、时长和格式，只分析、未下载。
- 解析器版本：启动更新框显示“稍后 / 更新”。经用户在最终删除确认时授权，前台删除下载版 `2026.7.4` 后只剩不可删除的内置 `2026.3.17`；随后从“下载最新版”重新下载成功，列表恢复 `2026.7.4`，并提示重启后生效。
...
### 2026-07-15 Android 1.0.1 任务标题与紧凑进度补充验收

- 本节覆盖并更新上文仍把主文件名列为任务卡片可见信息的旧描述；输出文件内部仍按媒体标题命名，但下载中和历史任务卡片不再显示文件名。
- 自动化覆盖历史映射、标题搜索、进度数值格式、未知大小、三类任务标题跑马灯、下载与硬盘图标以及右侧状态布局。独立只读审查指出并补强了滚动参数与图标尺寸的测试连接，复审无剩余问题。`:app:testDebugUnitTest --no-parallel` 共 `386/386` 通过；`:app:assembleDebug --no-parallel` 与 `:app:assembleDebugAndroidTest --no-parallel` 按顺序构建成功。
