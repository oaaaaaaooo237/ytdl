...
### 2026-07-10 当前主地址真实分析、下载完成与自动清理
...
- 历史页确认本轮完成记录后，按用户授权在前台删除记录；重启应用后队列显示 `正在下载(0)`、`等待中(0)`、`已完成(0)`、`失败(0)`，历史页为空。辅助核验 `run-as com.garyapp.ytdl find files/gui-downloads` 只剩根目录，无测试文件或任务目录。
