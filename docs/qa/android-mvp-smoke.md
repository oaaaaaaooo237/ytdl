...
- 版本：`versionCode=2`、`versionName=1.0.1`。底部导航合并为“下载 / 格式 / 任务 / 设置”四项，图标和文字均比 1.0.0 更醒目；“任务”页按顺序展示当前任务、等待任务和历史记录。
...
- 再次下载：取消的 YouTube 和 Eporner 记录均显示“再次下载”。点击后重新分析原地址并回到格式页，保留仍然有效的上次选择，不会直接开始下载。
...
- 自动验证：Python 全量测试通过；`:app:testDebugUnitTest --no-parallel`、`:app:assembleDebug --no-parallel`、`:app:assembleRelease --no-parallel`、`:app:assembleDebugAndroidTest --no-parallel` 均顺序通过。最终主 APK 和测试 APK 已覆盖安装到唯一可见的 `emulator-5554`。
